...
Below is a short snippet of code implementing this approach to topic analysis that you can use for inspiration. Happy topic modeling and exploration 😉 
...
def identify_topics(text, topic_labels, item_text):
    prompt = client.chat.completions.create(
        ...
        messages=[
            ...
            When analyzing employee feedback, always consider the wording of the survey item to which the employee
            responded to better understand the feedback. Here is the wording of the survey item: {item_text}       
            Here is the list of available topic labels: {','.join(topic_labels)}"""},
            {"role": "user", "content": f"""              
            First, summarize for yourself the feedback provided by the employee to make sure you have understood 
            it correctly.
            Then, from the list of available topic labels, select those that most accurately capture all important
            aspects of employee feedback. Avoid using topic labels that only weakly, i.e. indirectly or remotely,
            relate to the meaning of employee feedback. Use the topic label only if you are sure that it really
            captures one of the important aspects of employee feedback. Don't make things up and report only what 
            is really in the employee feedback.    
            ...
            employee feedback. New topic labels must be short, concise, not too general, but specific enough that
            the management can easily understand what the feedback is about. For example, a more specific topic
            label "HR Ticketing System Problems" would be better than a more general topic label "HR System
            Inefficiencies". 
