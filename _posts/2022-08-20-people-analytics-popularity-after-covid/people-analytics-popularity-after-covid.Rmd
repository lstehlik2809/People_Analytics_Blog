...
In addition to the main question addressed here, we may be interested in other insights provided by the model. To this end, we can plot the marginal effect of each predictor. Beyond what we already know, we can see that search interest in people analytics has clearly trended upwards over the last 15 years, and within each year, search interest follows the seasonality of work and holidays, i.e., search interest is lower during the holiday season - June, July, August and December - and higher throughout the rest of the year.   

```{r message=FALSE, warning=FALSE, results = "hidden", fig.show='hide' }
