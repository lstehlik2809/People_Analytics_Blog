---
title: 'RWA – A go-to tool for key drivers analysis of employee surveys data?'
description: |
...
As you can imagine, this makes RWA especially useful for key drivers analysis of employee survey data, when we want to determine the relative importance of (typically highly intercorrelated) survey items or indices for overall scores or external outcomes. 

One just needs to remember that these weights still reflect only association, not causation. Also, determining the statistical significance of individual relative weights isn’t trivial, since their sampling distribution isn’t well-defined, but with bootstrapping, it’s actually very easy to estimate corresponding confidence intervals.

Any helpful tips from folks who’ve used RWA more extensively?
...
Now let’s use a pair plot to examine the relationships between the individual scales. We’re particularly interested in the predictors of quitting intentions, but also in the correlations between other scales, as they can indicate how much RWA might help us address potential multicollinearity.
