...
\* *When estimating the explained variance of outcome variables by individual personality models, using the formula $R^2 = R_{xy}^T R_{xx}^{-1} R_{xy}$, where $R_{xx}$ is the correlation matrix of the predictor traits (e.g., the 5x5 matrix for the Big Five) and $R_{xy}$ is the vector of correlations between each trait and the outcome (CWB or task performance), I obtained a slightly higher estimate of 20.3%. This could be due to a small bug in the code (see the code snippet below), but given that the remaining three estimations are the same, it could instead be the result of a typo in the authors' paper* 🤔
